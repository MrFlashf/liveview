defmodule PentoWeb.ProductLive.FormComponent do
  ...
  defp upload_static_file(%{path: path}, socket) do
    dest = File.cwd!() <> Path.join("/priv/static/images", Path.basename(path))
    File.cp!(path, dest) |> IO.inspect()
    Routes.static_path(socket, "/images/#{Path.basename(dest)}") |> IO.inspect()
  end
